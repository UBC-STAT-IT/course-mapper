#!/usr/bin/env python3
"""
Hierarchical Course Layout Generator
Generates a hierarchical layout for course visualization with minimal edge crossings.
"""
...
def get_course_level(course_number: str) -> int:
    ...
    # Extract numeric part (remove first character like S, M, D)
    numeric_part = course_number[1:]
    
    # Get the hundreds digit
    level = int(numeric_part[0]) * 100
    
    # Return the actual level (we'll reassign based on prerequisites)
    return level
...
def assign_topological_levels(courses: List[str], prereq_graph: Dict[str, List[str]]) -> Dict[str, int]:
    ...
    for course, prereqs in prereq_graph.items():
        if course in courses:  # Only process courses in our set
            for prereq in prereqs:
                if prereq in courses:  # Only consider prerequisites that are also in our course set
    ...
    for course in courses:
        if in_degree[course] == 0:
            queue.append(course)
            levels[course] = 0  # Start at level 0
    ...
    while queue:
        ...
        for dependent in dependents[current]:
            ...
            # Assign level to dependent (at least one level higher than current)
    ...
    # Handle any remaining courses (cycles or disconnected components)
    for course in courses:
        if course not in levels:
            # Use original course level as fallback
            original_level = get_course_level(course)
            levels[course] = original_level // 100  # Convert to 0-based level
    
    # Apply special positioning rules
    # 1. S449 should be at the top (highest level)
    if 'S449' in levels:
        max_level = max(levels.values()) if levels else 0
        levels['S449'] = max_level + 1
    
    # 2. S307 and S308 should be on the same level (S308 is continuation of S307)
    if 'S307' in levels and 'S308' in levels:
        # Put S308 on the same level as S307
        levels['S308'] = levels['S307']
    
    return levels

def map_levels_to_display(topological_levels: Dict[str, int]) -> Dict[str, int]:
    """Map topological levels to display levels (100, 200, 300, 400, 500, etc.)."""
    # Get unique levels and sort them
    unique_levels = sorted(set(topological_levels.values()))
    
    # Create mapping to display levels
    level_mapping = {}
    for i, topo_level in enumerate(unique_levels):
        # Start at 100 and increment by 100
...
def count_crossings(courses_by_level: Dict[int, List[str]], 
    ...
    # Get all edges (prerequisite relationships)
    edges = []
    for course, prereqs in prereq_graph.items():
        course_level = get_course_level(course)
        for prereq in prereqs:
            prereq_level = get_course_level(prereq)
            # Only count edges between different levels
    ...
    for i, (edge1_from, edge1_to) in enumerate(edges):
        for j, (edge2_from, edge2_to) in enumerate(edges[i+1:], i+1):
            ...
            # Only check edges that could potentially cross
...
def barycenter_method(courses_by_level: Dict[int, List[str]], 
    ...
    # Initialize positions within each level
    positions = {}
    ...
    for level in levels:
        ...
        # Position courses by department (STAT, MATH, DSCI)
        pos = 0
        dept_order = ['S', 'M', 'D']  # STAT, MATH, DSCI
        
        for dept in dept_order:
            if dept in dept_groups:
                dept_courses = sorted(dept_groups[dept])
                
                # Special handling for S307 and S308 - ensure they are adjacent
                if 'S307' in dept_courses and 'S308' in dept_courses:
                    # Remove S307 and S308 from their current positions
                    dept_courses = [c for c in dept_courses if c not in ['S307', 'S308']]
                    # Insert them together in a good position (preferably at the start)
                    dept_courses = ['S307', 'S308'] + dept_courses
                
                for i, course in enumerate(dept_courses):
                    positions[course] = pos + i
                pos += len(dept_groups[dept]) + 1  # Add gap between departments
    
    # Apply barycenter method
    for iteration in range(max_iterations):
        new_positions = positions.copy()
        
        # Process levels from bottom to top, then top to bottom
        for direction in ['up', 'down']:
            ...
            for level in level_order:
                ...
                for course in courses:
                    # Calculate barycenter based on connected nodes
                    connected_positions = []
                    
                    if direction == 'up':
                        # Look at prerequisites (lower levels)
                        ...
                        # Look at dependents (higher levels)
                    ...
                    if connected_positions:
                        ...
                        barycenter = positions[course]  # Keep current position
...
def generate_coordinates(courses_by_level: Dict[int, List[str]], 
    ...
    # Y coordinates are distributed evenly across levels
    levels = sorted(courses_by_level.keys())
    ...
    # X coordinates based on positions within level
    for level, courses in courses_by_level.items():
        ...
        # Calculate x positions with spacing - center each level individually
        num_courses_in_level = len(courses)
        spacing = 2.0  # Horizontal spacing between courses
        
        if num_courses_in_level == 1:
            # Single course goes at center
            total_width = 0
        else:
            total_width = (num_courses_in_level - 1) * spacing
        
        for i, (_, course) in enumerate(level_courses_with_pos):
            # Center the courses horizontally for this specific level
            if num_courses_in_level == 1:
                x = 0  # Single course at center
            else:
...
def update_program_data(data: dict, coordinates: Dict[str, Tuple[float, float]], program_id: int = 1) -> dict:
    ...
    # Update requisites_program data
...
def main():
    """Main function to generate hierarchical layout."""
    print("Loading course data...")
    data = load_data('data/data.json')
    
    # Get courses from program 1 (assuming this is the main program)
    ...
    # Add any prerequisite courses that might not be in the main program
    ...
    # Categorize courses by their new levels
    courses_by_level = defaultdict(list)
    for course in all_courses:
        level = display_levels[course]
        courses_by_level[level].append(course)
    
    print(f"Courses by level (after topological sort):")
    for level in sorted(courses_by_level.keys()):
        dept_count = Counter(get_department(c) for c in courses_by_level[level])
        print(f"  {level}: {len(courses_by_level[level])} courses {dict(dept_count)}")
    
    print(f"Total levels needed: {num_levels} (was trying to fit into 4)")
    ...
    # Apply barycenter method to minimize crossings
    print("Applying barycenter method to minimize crossings...")
    ...
    new_data['layout_metadata'] = {
        'algorithm': 'hierarchical_topological_barycenter',
        ...
        'level_issues_fixed': len([c for c in all_courses]) - len(issues)
    }
    
    # Save to new file
    output_file = 'data/data_hierarchical.json'
    print(f"Saving hierarchical layout to {output_file}...")
    with open(output_file, 'w') as f:
        json.dump(new_data, f, indent=2)
    
    print(f"✅ Hierarchical layout generated successfully!")
